...
import { CloseHookPurchaseButton } from "./CloseHookPurchaseButton";
...
export default async function FaenaDetailsPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const isHookPurchase = slaughter.batch.isHookPurchase;
  ...
  const totalSlaughterHeads = slaughter.details.length;
  ...
  let availableItems = slaughter.batch.details.map(d => d.item).filter((v,i,a)=>a.findIndex(t=>(t.id === v.id))===i);
  if (isHookPurchase) {
    availableItems = await prisma.item.findMany({ where: { isSlaughterable: true, status: true } });
  }
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-2xl font-bold text-zinc-100">
              Faena - Lote #{slaughter.batch.batchNumber} {isHookPurchase && <span className="text-rose-400 text-sm border border-rose-500/30 bg-rose-500/10 px-2 py-0.5 rounded-full ml-2">Compra al Gancho</span>}
            </h1>
        ...
        <div className="flex items-center gap-3">
          <MenudenciaForm 
            ...
            maxQuantity={isHookPurchase ? Math.ceil(totalFaenaHeads) : totalBoughtHeads} 
            existingQuantity={slaughter.batch.inventoryLots.find(l => l.item.code === "MENUDENCIA-LOTE")?.initialStock || 0}
            isAdmin={userRole === "ADMIN"}
          />
          {isOpen ? (
            isHookPurchase ? (
              <CloseHookPurchaseButton 
                slaughterId={slaughter.id}
                slaughterDetails={slaughter.details}
      ...
      <div className={`grid grid-cols-1 ${isHookPurchase ? 'lg:grid-cols-2' : 'lg:grid-cols-3'} gap-6`}>
        ...
        <div className={`lg:col-span-2 space-y-6 ${isHookPurchase ? 'lg:col-span-2 lg:mx-auto lg:w-full lg:max-w-4xl' : ''}`}>
          {isOpen && (
            <FaenaForm 
              slaughterId={slaughter.id} 
